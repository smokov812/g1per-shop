...
def main_menu_keyboard(is_admin: bool = False) -> ReplyKeyboardMarkup:
    ...
    builder.row(KeyboardButton(text=CATALOG_BUTTON), KeyboardButton(text=CART_BUTTON))
    if is_admin:
        builder.row(KeyboardButton(text=ADMIN_PANEL_BUTTON))
...
def categories_keyboard(categories) -> InlineKeyboardMarkup:
    ...
    for category in categories:
        builder.button(text=category.title, callback_data=f"user:category:{category.id}")
    builder.adjust(1)
...
def product_keyboard(product) -> InlineKeyboardMarkup:
    ...
    if product.category_id:
        builder.button(text="Назад к товарам", callback_data=f"user:category:{product.category_id}")
    else:
        builder.button(text="К категориям", callback_data="user:categories")
    builder.adjust(1)
    return builder.as_markup()
