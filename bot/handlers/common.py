...
async def _start_prefix_entity(message: Message) -> tuple[str, list[MessageEntity]]:
    text = START_CUSTOM_EMOJI_FALLBACK
    entities = [
        MessageEntity(
            type="custom_emoji",
            offset=0,
            length=_utf16_len(START_CUSTOM_EMOJI_FALLBACK),
...
def get_common_router(
    ...
    *,
    ...
    async def cmd_start(message: Message) -> None:
        ...
        prefix_text, prefix_entities = await _start_prefix_entity(message)
        text = (
            f"{prefix_text} Добро пожаловать, <b>{username}</b>\n\n"
        ...
        if BANNER_PATH.exists():
            try:
                await message.answer_photo(
                    ...
                    caption=text,
                    caption_entities=prefix_entities,
        ...
        try:
            await message.answer(text, entities=prefix_entities, reply_markup=keyboard)
        except TelegramBadRequest as exc:
            logger.warning("Failed to send custom emoji start text: %s", exc)
            fallback_text = (
