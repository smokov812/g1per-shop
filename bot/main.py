...
from bot.services import create_payment_service
...
def validate_runtime_config(config: Config) -> None:
    ...
    if config.payment_provider == "cryptomus":
        ...
        if config.cryptomus_webhook_enabled and not config.cryptomus_webhook_url:
            raise RuntimeError("При включенном webhook нужно заполнить CRYPTOMUS_WEBHOOK_URL.")
        if not config.cryptomus_webhook_enabled:
            logger.warning("Cryptomus запущен без webhook. Будет работать только polling fallback, это хуже для production.")
...
async def payment_sync_worker(*, bot: Bot, config: Config, session_maker, payment_service) -> None:
    if not config.payment_sync_enabled or not payment_service.supports_status_polling():
        return

    logger.info(
        "Payment sync worker enabled: interval=%ss stale=%ss batch=%s",
    ...
    try:
        while True:
            ...
            for payment in payments:
                try:
                    payload = await payment_service.fetch_payment_status(
                        external_payment_id=payment.external_payment_id,
                        order_id=payment.order_id,
                    )
                except Exception as exc:
                    logger.warning("Payment sync failed for payment_id=%s: %s", payment.id, exc)
                    async with session_maker() as session:
                        await PaymentRepository(session).mark_sync_error(payment.id, str(exc))
                    continue

                async with session_maker() as session:
                    result = await PaymentRepository(session).process_cryptomus_callback(payload, source="poll")
...
async def run_polling() -> None:
    ...
    payment_service = create_payment_service(config)
    ...
    sync_task = None
    if config.payment_sync_enabled and payment_service.supports_status_polling():
